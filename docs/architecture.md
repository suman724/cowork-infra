...
**Standard event names:** `session_created`, `session_started`, `step_started`, `step_completed`, `step_limit_approaching`, `text_chunk`, `llm_request_started`, `llm_request_completed`, `tool_requested`, `tool_completed`, `approval_requested`, `approval_resolved`, `policy_expired`, `task_completed`, `task_failed`, `session_completed`, `session_failed`
...
**Sandbox Tasks:**
Cloud sandbox sessions run as an **ECS Service worker pool**. Session Service publishes session requests to an SQS queue; idle worker tasks poll the queue, pick up sessions, and serve them one at a time. Each task terminates after its session ends; ECS replaces it to maintain desired count. Utilization-based auto-scaling (`scale_in_enabled = false`) ensures idle tasks are always available for instant session pickup. The `sandbox` Terraform module (`iac/modules/sandbox/`) provisions the ECS Service, SQS queue, task definition, security group, and IAM roles. The agent runtime Docker image is built from `cowork-agent-runtime/Dockerfile` — a multi-stage build on `python:3.12-slim` that runs as a non-root user, exposes port 8080 for `HttpTransport`, and uses `/workspace` for file sync. See [design/sqs-sandbox-dispatch.md](design/sqs-sandbox-dispatch.md) for the full design and [components/local-agent-host.md, Section 13](components/local-agent-host.md#13-container-deployment) for container deployment details.
...
1. Browser → `POST /sessions` with `executionEnvironment: "cloud_sandbox"` → Session Service creates session in `SANDBOX_PROVISIONING`, publishes to SQS queue, creates `cloud`-scoped workspace
2. Idle worker task polls SQS, receives session message, deletes it, reads container IP from ECS metadata endpoint, calls `POST /sessions/{id}/register` with `sandboxEndpoint` and `registrationToken` → Session Service validates token (single-use via conditional update), returns policy bundle + workspace ID, transitions to `SANDBOX_READY`
