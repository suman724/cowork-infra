...
|-------------|------|-------------|
| Session resume | 15 | Always load history in `init_from_registration()` (no-op for new sessions, loads thread for resumed), add SQS dispatch to resume endpoint, web app reconnection UX. No schema changes needed. |
